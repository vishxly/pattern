...
console.log(data);`,
  ...
  const tabs: TabInfo[] = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none">
          <path
            d="M17.5 2L7 12l-4.5-3.5L1 10l6 6 11.5-12L17.5 2z"
            fill="currentColor"
          />
        </svg>
      ),
      ...
      <div className="font-mono text-sm leading-relaxed">
        {lines.map((line, index) => (
          <div key={index} className="flex">
            ...
            <div className="flex-1">{highlightSyntax(line, language)}</div>
          </div>
        ))}
      </div>
    );
  ...
  const highlightSyntax = (line: string, language: Language) => {
    ...
    highlightedLine = highlightedLine.replace(
      /(["'`])((?:\\.|(?!\1)[^\\])*?)\1/g,
      '<span class="text-emerald-600 dark:text-emerald-400">$1$2$1</span>'
    );
